...
<center>
	...
	$e=$d+1;
	$sql="SELECT * FROM abcd where no='$d'";
	$result=$conn->query($sql);
	$row=$result->fetch_array();
	$a=$row['count']+1;
	...
	if ($a==5){
		...
		$sql="UPDATE record SET progress='$e' where s_id=$login_id";
		$conn->query($sql);
		$sql="UPDATE abcd SET count=0 where count=5";
		$conn->query($sql);
	}
	if($a==5){
		if($e==27){
			$sql="UPDATE record SET progress=1 where s_id=$login_id";
			$conn->query($sql);
		}
	}
	$sql="UPDATE abcd SET count='$a' WHERE no='$d'";
	$conn->query($sql);

	$pic=$row['pic'];
	echo "<img src='$pic' width=20% height='auto'/>";

	echo "<textarea id='note-textarea'  placeholder='type something' cols='35' rows='11'>";
	echo $row['speech'];
	echo "</textarea>";
